...
/**
 * Реализация алгоритма проверки планарности графа и его максимальной планарности,
...
public class IsMaximallyPlanar implements GraphProperty {
    ...
    public boolean run(Graph graph) {
        ...
        // Тривиальный случай: пустой граф или граф без рёбер планарен (и максимально планарен, если n=0,1)
        if (n <= 0) return true;

        // Алгоритм предназначен для неориентированных графов
        if (graph.isDirect()) return false;

        // Обработка малых графов: K1, K2. Они максимальны планарны, если полные.
        if (n <= 2) return m == n * (n - 1) / 2;
        ...
        // Сначала проверяем, является ли граф просто планарным
        if (!isPlanar(graph)) {
            return false;
        }

        // Если граф планарен, проверяем условие максимальной планарности:
        // для планарного графа с n > 2 вершинами, он максимально планарен, если m = 3n - 6.
        return m == 3 * n - 6;
    }
